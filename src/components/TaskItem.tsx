...
const TaskItem: React.FC<TaskItemProps> = ({ title, startTime, endTime, date, completed, onClick, inputProps }) => {
      ...
      <div className="flex items-center justify-between cursor-pointer" onClick={onClick}>
        <div  className="flex items-center md:gap-3">
          ...
          <div className="text-skin-text-primary">
            <h3 style={{whiteSpace: 'normal'}} className={`font-medium text-[14px] ${completed ? 'line-through text-skin-text-done' : ''}`}>{title}</h3>
            <p className={`font-normal text-[14px] ${completed ? 'line-through text-skin-text-done' : ''}`}>{startTime + ' - ' + endTime}</p>
          </div>
        </div>
        <p className="text-skin-text-gray font-medium text-[14px]">{`${!date.month || formatedDate === today ? 'Toady' : formatedDate}`}</p>
      </div>
    </Card>
  );
};
